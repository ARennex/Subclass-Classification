...
def experiment(files, Y, classes, N, n_splits):
    ...
    for activation in activations:
        ...
        for model_name in [0,1,2,3]:

            # load json and create model
            json_file = open(base_path + '/Results2Fold/tanh/model/'+str(model_name)+'.json', 'r')
            loaded_model_json = json_file.read()
            json_file.close()
            loaded_model = model_from_json(loaded_model_json)
            # load weights into new model
            loaded_model.load_weights(base_path + "/Results2Fold/tanh/model/"+str(model_name)+".h5")
            ...
            print(df_confusion)
            df_confusion.to_csv(('Results2Fold/Data Points per Class, Model '+ str(model_name) + '.csv'))
            ...
            plt.savefig('Results2Fold/Percentage Accuracy in each bin, Model '+ str(model_name) + '.png')
            plt.close()
...
while NUMBER_OF_POINTS <= MAX_NUMBER_OF_POINTS:
    ...
    text_file = open("Results2Fold/Accuracy last model.txt", "a")
