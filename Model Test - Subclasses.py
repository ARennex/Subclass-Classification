...
## Settings
# Files Setting
limit = 20000 # Maximum amount of Star Per Class Per Survey
extraRandom = True
permutation = False # Permute Files
...
regular_exp2 = base_path + '/Temp/VVV/**/*.csv'
regular_exp3 = base_path + '/Temp/ASASSN/**/**/*.csv'
    ...
    for file, num in files:
        ...
        if c in subclasses:
            if 'VVV' in file:
                ...
                t, m, e, length = open_asassn(file, num, N, [1,2,3])
            if c in subclasses:
...
def plot_confusion_matrix(cm,
    ...
    plt.savefig("ResultsSubclasses/Confusion_Matrix_Subclass_ver" + str(model_iter) + ".png",bbox_inches='tight')
    #plt.show()
    plt.clf()
...
def experiment(files, Y, classes, N, n_splits):
    ...
    for activation in activations:
        ...
        for model_name in [0,1]:
            ...
            plot_confusion_matrix(cm           = df_confusion.to_numpy(),
                                  normalize    = True,
                                  target_names = ['Mira','OSARG','RRab','RRc','SRV','clasCep','contactBinary','detachedBinary','dsct','t2Cep'],
                                  title        = "Confusion Matrix",
            ...
            print(df_confusion)
            df_confusion.to_csv(('ResultsSubclasses/Data Points per Subclass, Model '+ str(model_name) + '.csv'))
...
while NUMBER_OF_POINTS <= MAX_NUMBER_OF_POINTS:
    ...
    text_file = open("ResultsSubclasses/Subclass accuracy last model.txt", "a")
